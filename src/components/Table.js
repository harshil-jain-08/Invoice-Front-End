...
import { ArrowLeftCircle, ArrowRightCircle, PencilSquare } from "react-bootstrap-icons";
...
const TableHeader = ({ heads }) => {
    return heads.map((data) => {
        ...
        return <th key={data} width={data === "description" ? "40%" : "auto"} > {data.toUpperCase()}</th >
    })
}

const TableBody = ({ results, heads, from, setter, setShow, changeStat }) => {
            ...
            <tr key={val["id"]} onClick={() => {
            ...
            }}>
                {
                    heads.map((v) => {
                        ...
                        if (v === "paid status") return <td key={val["paid status"]}><span className={val[v]} onClick={() => changeStat(val["id"], val["amount"])}>{val[v]}</span></td>
                        return <td key={v}>{val[v]}</td>
                    })
                }
                {from === "selector" ? <></> : <td key={"edit"}><PencilSquare size={"1.5em"} onClick={() => { edit(val["id"]) }} /></td>}
            </tr>
        )
    });
}
...
const Table = ({ data, from, setter }) => {
    const [page, setPage] = useState(1)
    const { setShow, changeStat } = useContext(InvoiceContext);
    const [search, setSearch] = useState("")
    if (data === null || data.length === 0) {
        return <table><tbody><tr><td>No Entries</td></tr></tbody></table>
    }
    ...
    const pageData = results.slice((page - 1) * 6, Math.min(results.length, page * 6));
    ...
    return (
        <>
            ...
            <div className="fixed-table">
                <table>
                    <thead>
                        <tr><TableHeader heads={heads} />
                            <th key={"edit"} style={{ width: "5%" }}></th>
                        </tr>
                    </thead>
                    <tbody>
                        <TableBody results={pageData} heads={heads} from={from} setter={setter} setShow={setShow} changeStat={changeStat} />
                    </tbody>
                </table>
                <span className="page">
